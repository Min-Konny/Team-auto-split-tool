...
import LobbyPasteModal from '@/components/LobbyPasteModal'
import SplitModePickerModal from '@/components/team-maker/SplitModePickerModal'
import TeamSplitOverlay from '@/components/team-maker/TeamSplitOverlay'
import {
  createLobby,
  getLobby,
  isLobbyExpired,
...
import { generateThreeCandidates, SplitCandidate, Teams } from '@/lib/teamBalancer'
...
export default function TeamMakerPage() {
  ...
  const lastSplitKeyRef = useRef<string | null>(null)
  const lastLobbyKeyRef = useRef<string | null>(null)
  ...
  const filtered = useMemo(
    () =>
      members.filter((p) => {
        ...
        return mSearch && mLobby
      }),
    [members, search, lobbyOnly, lobby?.checkInIds.length, checkInSet]
  )
  ...
  // ロビーが10人揃ったら選択に反映（入室ログ貼付・リンク参加など）
  useEffect(() => {
    const count = lobby?.checkInIds.length ?? 0
    if (count < 10 || members.length === 0) {
      if (count < 10) lastLobbyKeyRef.current = null
      return
    }
    const key = lobby!.checkInIds.slice(0, 10).join(',')
    if (key === lastLobbyKeyRef.current) return
    lastLobbyKeyRef.current = key
    applyLobbyToSelected(lobby!.checkInIds)
  }, [lobby?.checkInIds, members.length, applyLobbyToSelected])

  const handlePoolClick = async (member: Member, e: React.MouseEvent) => {
    if (!member.id) return

    // Shift+クリック: ロビーチェックイン（共有リンク用）
    if (e.shiftKey && lobby?.id && lobby.status === 'open' && communityId) {
      setLobbyBusy(true)
      try {
    ...
    if (isSelected(member.id)) {
      setSelected((prev) => prev.filter((s) => s.member.id !== member.id))
      return
    }
  ...
  const runSplit = useCallback((list: SplitCandidate[]) => {
    setCandidates(list)
  ...
  const runTeamSplit = useCallback(
    (memberList: Member[], force = false) => {
      if (memberList.length < 10) return
      const key = memberList
        .map((m) => m.id)
        .sort()
        .join(',')
      if (!force && key === lastSplitKeyRef.current) return
      lastSplitKeyRef.current = key
      try {
        const list = generateThreeCandidates(memberList)
        runSplit(list)
      } catch (e) {
        console.error(e)
      }
  ...
  const createTeams = () => runTeamSplit(selected.map((s) => s.member), true)

  // 10人揃ったら自動チーム分け / 10人未満ならリセット
  useEffect(() => {
    if (selected.length < 10) {
      lastSplitKeyRef.current = null
      setShowOverlay(false)
      setShowModePicker(false)
      setTeams(null)
      setCandidates(null)
    ...
    runTeamSplit(selected.map((s) => s.member))
  }, [selected, runTeamSplit])
    ...
    try {
      ...
      const res = await fetch('/api/match/register', {
        ...
        body: JSON.stringify({
          winner,
          players,
          balanceScore: candidates?.[activeCandidate]?.metrics.balanceScore,
    ...
    <div>
      ...
      <section className="lobby-bar">
        <div className="lobby-bar-inner">
          ...
          <div className="lobby-actions lobby-actions-end">
            <label className="lobby-filter" title="ロビーにチェックインした人だけ左のリストに表示">
              <input type="checkbox" checked={lobbyOnly} onChange={(e) => setLobbyOnly(e.target.checked)} />
              参加者のみ表示
            </label>
            {lobbyCount >= 10 && (
              <span className="lobby-ready">10人揃い — 自動でチーム分け</span>
            )}
          </div>
        </div>
        {lobbyCount > 10 && (
          <p className="lobby-warn">ロビーが10人を超えています。先頭10人でチーム分けします。</p>
        )}
        {lobby?.inviteToken && (
      ...
      <div className="page-layout">
        <section className="panel">
          ...
          <p className="pool-hint">クリックで選択（10人で自動チーム分け）· Shift+クリックでロビー参加</p>
          ...
          <div className="panel-body">
            {selected.length === 0 && (
              <div className="empty-msg">
                左のリストから10人選ぶと自動でチーム分け<br />
                <span className="empty-sub">ロビー・入室ログ貼付で10人揃っても同様</span>
              </div>
            )}
            {selected.map((sp, i) => (
      ...
      <div className="action-footer">
        <div className="footer-inner">
          ...
          {teams && !showOverlay && (
            <button type="button" className="btn-primary" onClick={() => setShowOverlay(true)}>
              チーム表示
            </button>
          )}
          {candidates && !teams && !showModePicker && (
      ...
      {candidates && (
        <SplitModePickerModal
          candidates={candidates}
          open={showModePicker}
          onPick={applySplitMode}
      ...
      {showOverlay && teams && candidates && (
        <AnimatePresence>
          <TeamSplitOverlay
            ...
            onRegenerate={createTeams}
            onPickCandidate={pickCandidate}
            onSwapClick={handleSwapClick}
            onSwapCancel={() => setSwapSource(null)}
            onRegister={registerMatch}
          />
        </AnimatePresence>
      )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .lobby-ready{font-size:12px;font-weight:600;color:var(--ok);font-family:'JetBrains Mono'}
        ...
        .cand-tabs-wrap{padding:12px 28px 0;border-bottom:1px solid var(--line)}
        .cand-tabs-label{display:block;font-family:'JetBrains Mono';font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:var(--fg-3);margin-bottom:8px}
        .cand-tabs{display:flex;gap:8px;padding-bottom:12px;flex-wrap:wrap}
        ...
        .pool-hint{margin:0;padding:0 20px 10px;font-size:11px;color:var(--fg-3);font-family:'JetBrains Mono'}
        ...
        .empty-sub{display:block;margin-top:6px;font-size:11px;color:var(--fg-3);opacity:.85}
        ...
        .ov-actions{margin-left:auto;display:flex;gap:8px}.icon-btn{width:34px;height:34px;border-radius:8px;border:1px solid var(--line);background:transparent;color:var(--fg-2)}.copy-btn{padding:8px 15px;border-radius:8px;border:1px solid color-mix(in oklch,var(--blue) 40%,var(--line));background:color-mix(in oklch,var(--blue) 10%,transparent);color:var(--blue);font-family:'Space Grotesk';font-weight:600;font-size:13px;cursor:pointer;white-space:nowrap}.copy-btn:disabled{opacity:.35;cursor:not-allowed}.regen-btn{padding:8px 15px;border-radius:8px;border:1px solid var(--line);background:transparent;color:var(--fg-1)}
        ...
        .swap-banner{background:color-mix(in oklch,var(--gold) 10%,var(--bg-1));border:1px solid color-mix(in oklch,var(--gold) 40%,transparent);border-radius:10px;padding:10px 16px;font-family:'JetBrains Mono';font-size:12px;color:var(--gold);display:flex;align-items:center;gap:10px;flex-wrap:wrap}
        .swap-cancel{margin-left:auto;background:transparent;border:1px solid color-mix(in oklch,var(--gold) 40%,transparent);color:var(--gold);border-radius:5px;padding:3px 10px;font-family:'JetBrains Mono';font-size:10px;cursor:pointer}
        .role-row{display:grid;grid-template-columns:1fr 84px 1fr;gap:8px;align-items:stretch}
        .rr-card{background:var(--bg-1);border:1px solid var(--line);border-radius:10px;padding:14px 20px;display:flex;align-items:center;gap:14px;height:100%;cursor:pointer;transition:border-color .15s,box-shadow .15s}
        .rr-card:hover{border-color:var(--line-2);box-shadow:0 4px 16px color-mix(in oklch,var(--fg-0) 6%,transparent)}
        ...
        .rr-card.is-source{border-color:var(--gold);box-shadow:0 0 0 1px var(--gold)}.rr-card.is-target{border-style:dashed;border-color:color-mix(in oklch,var(--blue) 50%,var(--line));background:color-mix(in oklch,var(--blue) 5%,var(--bg-1))}
